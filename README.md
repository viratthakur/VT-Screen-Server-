<img width="1727" height="911" alt="MAIN Podt" src="https://github.com/user-attachments/assets/d6f93eec-5341-4fe3-9da3-ac22b243daad" />

 # ⚠️ Disclaimer 
 This tool is intended for personal use only.  Only run this on your own device.  Do not use on any system without explicit permission.


## Prerequisites

| Requirement |         |
|-------------|---------|
| Python | PIP3         |
| Modul  | Mass Pilo Flask    |
| OS | Linux / Windows  |


# RUN WINDOWS CMD NOT USING POWERSHELL 

<img width="1910" height="930" alt="Windows " src="https://github.com/user-attachments/assets/0384288d-7840-436c-8386-70cb9cc0b290" />

# WINDDOWS 
```bash
01
#Python --version  
pip install flask mss pillow
pip install flask-sock
pip3 install flask flask-sock mss pillow opencv-python numpy --break-system-packages
## Installation
02
 # UNZIP Folder --> File VT-Screen-Server--main
------> Open CMD < -------- 
 cd C:\Users\virat\Desktop\RHCSA\Github\VT-Screen-Server--main\VT-Screen-Server--main
 python "server 03.py"

```



# KaliLinux Deb Package 
Python 3 support in kalilinux 
<img width="1645" height="844" alt="kalilnux screen Shoot" src="https://github.com/user-attachments/assets/4d081875-896d-440b-b3c3-fbbb816de8c8" />




## Prerequisites
```bash
 python --version
 pip3 install flask flask-sock mss pillow opencv-python numpy --break-system-packages
 python3 server\ 03.py
 
```


## Special Thanks ❤️ 
This project was inspired by a feature request from one of my YouTube subscribers. The implementation and development were completed by me.


## YouTube

📺 [https://www.youtube.com/@VIRATH4K3R](https://www.youtube.com/@VIRATH4K3R896600)

## Connect with Me

- 📺 **YouTube:** https://www.youtube.com/@VIRATH4K3R
- 💻 **GitHub:** https://github.com/viratthakur



## Copyright

© 2026 VIRAT THAKUR. All Rights Reserved.

This project is published for portfolio and educational demonstration purposes.
No part of this source code may be copied, redistributed, or used without permission.

